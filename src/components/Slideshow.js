    ...
    <div className="relative overflow-hidden">
      <AnimatePresence mode="popLayout">
        ...
        >
          ...
          />
          <div className="absolute inset-0 bg-lime-700 opacity-70"></div>
        </motion.div>
      </AnimatePresence>
      <AnimatePresence>
        <motion.div
      ...
      <div className="absolute bottom-10 left-64 flex space-x-2">
        {Array.from({ length: feats.length }).map((_, index) => (
          <button onClick={()=> setFeatNum(index)} key={index} className={`${index === featNum ? "w-12" : "w-5"} transition-all rounded-full h-2 bg-white duration-500 ease-in-out`}/>
        ))}
      </div>
    </div>
  );
